...
#       @error_messages.push(["'{' expected at the beginning of the line", @line_number])
#     end
#   end
#
#   def after_curly
#     if @this_line.include?('{') && @this_line[@this_line.index('{') + 1] != "\n" && @this_line[@this_line.index('{') + 1] != "\n"
#       @error_messages.push(["New line expected after '{'",
#                             @line_number])
#     end
#     if @this_line.include?('}') && @this_line[@this_line.index('}') + 1] != "\n" && @this_line[@this_line.index('}') + 1] != "\n"
#       @error_messages.push(["New line expected after '}'",
...
#       @this_line2 = @this_line.split('"')
#       @this_line2.each_with_index do |split, i|
#         if split == ',' && !@this_line2[i + 1].nil? && (split[split.index(',') + 1] != "\n")
#           @error_messages.push(["New line expected after ','", @line_number])
#         end
#       end
#     end
#   end
...
#       @this_line2 = @this_line.split('"')
#       @this_line2.each_with_index do |split, _i|
#         if split.include?('[') && (split[split.index('[') + 1] != "\n")
#
#           @error_messages.push(["New line expected after '['", @line_number])
#         end
#       end
#     end
#     if @this_line.include?('],') && !(@this_line[@this_line.index(']') + 1] == "\n" || @this_line[@this_line.index(']') + 1] == ',') && !(@this_line[@this_line.index(']') + 1] == "\n" || @this_line[@this_line.index(']') + 1] == ',')
#       @error_messages.push(["Empty line is expected after '],'",
#                             @line_number])
#     end
#   end
#
...
#       before_split = @this_line[@this_line.index('}') - 1]
#       unless ["\n", ' '].include?(before_split)
#         @error_messages.push(["New empty line expected before '}'",
#                               @line_number])
#       end
#     end
#   end
...
#         @error_messages.push(["Indentation 0 is expected for '{' and '} ",
#                               @line_number])
#       end
#     elsif @this_line.include?('[') || @this_line.include?(']')
#
